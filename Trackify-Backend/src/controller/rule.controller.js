...
const toggleOvertime = async (req, res) => {
  ...
  try {
    ...
    // Toggle the isOvertime property
    existingRule.isOvertime = !existingRule.isOvertime;
    await existingRule.save();
    ...
    return res
      .status(500)
      .json({ error: `Failed to update Rule: ${error.message}` });
  }
};
